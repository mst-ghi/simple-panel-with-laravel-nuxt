<template>
  <div>
    <Header title="لیست شهر های ثبت شده در سیستم">
      <div>
        ...
        <v-toolbar flat color="dark">
          <v-dialog v-model="dialog" max-width="900px">
            <template v-slot:activator="{ on }">
              <v-btn v-if="$can('city_store')" class="ma-2 white--text" v-on="on">
                <v-icon left dark>mdi-plus-outline</v-icon>شهر جدید
              </v-btn>
            </template>
            <v-card>
              <v-card-title>
                  ...
                  <v-row>
                    ...
                    <v-col cols="12" sm="6" md="4">
                      <v-select
                        ...
                        label="شهرستان"
                        :error="errors.county_id != null"
                        :error-messages="errors.county_id"
                        clearable
                      ></v-select>
                    </v-col>
                    <v-col cols="12" sm="6" md="4">
...
export default {
  ...
  computed: {
    formTitle() {
      return this.editedIndex === -1 ? "شهر جدید" : "ویرایش شهر";
    }
  },
  ...
  methods: {
    ...
    async initialize() {
      ...
      await this.$axios
        .get(
          `api/panel/cities?per_page=${this.options.per_page}&page=${this.options.current}`
        )
        .then(response => {
    ...
    async deleteItem() {
      this.destroy = false;
      await this.$axios
        .delete(`api/panel/cities/${this.itemDestroy.id}`)
        .then(response => {
    ...
    async save() {
      ...
      if (this.editedIndex > -1) {
        await this.update();
        this.selectedCounty = this.editedItem.county_id;
    ...
    async update() {
      ...
      await this.$axios
        .patch(`api/panel/cities/${id}`, data)
        .then(response => {
          if (response.data.success) {
            this.$toast.success(response.data.message);
