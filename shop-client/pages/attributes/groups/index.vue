<template>
  <div>
    <Header title="لیست گروه خصوصیات ثبت شده در سیستم">
      <div>
    ...
    >
        ...
        <v-toolbar flat color="dark">
          <v-dialog v-model="dialog" max-width="600px">
            <template v-slot:activator="{ on }">
              <v-btn v-if="$can('attribute_group_store')" class="ma-2 white--text" v-on="on">
                <v-icon left dark>mdi-plus-outline</v-icon>گروه خصوصیات جدید
              </v-btn>
            </template>
            <v-card>
              <v-card-title>
      ...
      <template v-slot:item.action="{ item }">
        <v-icon
          dense
          v-if="$can('attribute_group_update')"
          class="ml-2"
          @click="editItem(item)"
        >mdi-circle-edit-outline</v-icon>
        <v-icon
          dense
          v-if="$can('attribute_group_destroy')"
...
export default {
  ...
  methods: {
    ...
    async initialize() {
      ...
      await this.$axios
        .get(
          `api/panel/attributes/groups?per_page=${this.options.per_page}&page=${this.options.current}`
        )
        .then(response => {
    ...
    async deleteItem() {
      this.destroy = false;
      await this.$axios
        .delete(`api/panel/attributes/groups/${this.itemDestroy.id}`)
        .then(response => {
    ...
    async store() {
      ...
      await this.$axios
        .post(`api/panel/attributes/groups`, data)
        .then(response => {
          if (response.data.success) {
            this.$toast.success(response.data.message);
            this.desserts.push(response.data.data);
            this.attribute_group_count++;
            this.close();
          } else {
            this.$toast.error(response.data.message);
          }
    ...
    async update() {
      ...
      await this.$axios
        .patch(`api/panel/attributes/groups/${id}`, data)
        .then(response => {
          if (response.data.success) {
            this.$toast.success(response.data.message);
